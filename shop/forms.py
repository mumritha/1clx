...
def validate_slug(value):
    ...
    existing_slugs = Product.objects.filter(deleted_at__isnull=True).values_list('slug', flat=True)
    if value in existing_slugs:
...
class ProductForm(forms.Form):
    ...
    Stock = forms.ChoiceField(choices=[(1, 'in_stock'), (2, 'out_of_stock')], widget=forms.RadioSelect(), initial=1)
    stockqty = forms.IntegerField()
    visibility = forms.ChoiceField(choices=[(1, 'published'), (2, ' hidden')], widget=forms.RadioSelect(), initial=1) 
